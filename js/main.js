...
var magentaIcon = L.icon({                              // custom map icon 1 in meganta
    iconUrl: 'images/magenta_ring.png',
    shadowUrl: null,
    iconSize:     [8,8], // size of the icon
});

var greenIcon = L.icon({                               // custom map icon 2 in green
    iconUrl: 'images/green_ring.png',
    shadowUrl: null,
    iconSize:     [8,8], // size of the icon
});

var whiteIcon = L.icon({                               // custom map icon 3 in white
    iconUrl: 'images/white_dot.png',
    shadowUrl: null,
    iconSize:     [8,8], // size of the icon
});
...
// PLEASE READ: This version of the code pulls down 1,000 data objects with point features from 1006 to 2016
// It then filters those by crime time between Fraud and Theft
// The original plan was to bring down 1,000 points from 2016 and then filter that in more interesting ways
// I also wanted to have the map pan to specific areas on button clicks.
// Because I didn't have time to build in more interesting filters or the pan options, I'm submitting this version, which is boring.
// The code for the 2016 data is below, in the ajax section, commented out.
...
$.ajax({
    ...
    type: "GET",
    data: {
      "$query" : "SELECT * WHERE shape is not null",                                                       // This filters out any data from the pullet set without a shape component, so all 1000 data objects should be mappable
...
////////////////////////////////////////////////////////////////////////////////////////
    ...
    var makeMarkersWhite =  function (crimeData) {                                      // markers using the white custom icon
        var markersTempWhite = _.map(crimeData, function(crimeObject) {
          return L.marker([crimeObject.shape.coordinates[1], crimeObject.shape.coordinates[0]], {icon: whiteIcon})
          .bindPopup(crimeObject.text_general_code + " reported at " + crimeObject.location_block + " on " + crimeObject.dispatch_date);
        }
      );
      return markersTempWhite;
    };
    ...
    var allMarkers = makeMarkersWhite(crimeData);
